...
def random_sparse_env_small(random_seed, max_width, max_height, observation_builder):
    ...
    while width <= max_width and height <= max_height:
        try:
            ...
            print(
                "[{}] {}x{} {} cities {} trains, max {} rails between cities, max {} rails in cities. Malfunction rate {}, {} to {} steps.".format(
            ...
            width += 5
            height += 5
            logging.info("Try again with larger envs: (w,h):", width, height)
    logging.error(f"Unable to generate envs with seed={random_seed}, max_width={max_height}, max_height={max_height}")
    return None
