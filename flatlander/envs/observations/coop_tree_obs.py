...
from flatlander.envs.observations.builders.coop_tree import CoopTreeObs
from flatlander.envs.observations.utils import _get_small_node_feature_vector, _get_coop_node_feature_vector


@register_obs("coop_tree")
class CoopTreeObservation(Observation):
    PAD_VALUE = -np.inf

    def __init__(self, config) -> None:
        super().__init__(config)
        self._builder = CoopTreeObsWrapper(
            CoopTreeObs(
...
class CoopTreeObsWrapper(ObservationBuilder):

    def __init__(self, tree_obs_builder: CoopTreeObs, small_tree=False):
        super().__init__()
    ...
    def build_obs(self, obs_node: Node):
        padded_observations = np.full(shape=(self.max_nr_nodes, self.observation_dim,),
                                      fill_value=CoopTreeObservation.PAD_VALUE)
    ...
    def dfs(self, node: Node,
        ...
        node_obs = _get_small_node_feature_vector(node) if self._small_tree else _get_coop_node_feature_vector(node)
